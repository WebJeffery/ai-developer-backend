...
async def get_obj_detail_controller(
    ...
    auth: AuthSchema = Depends(AuthPermission(["demo:example:query"]))
) -> JSONResponse:
...
async def create_obj_controller(
    ...
    auth: AuthSchema = Depends(AuthPermission(["demo:example:create"]))
) -> JSONResponse:
...
async def update_obj_controller(
    ...
    id: int = Path(..., description="示例ID"),
    auth: AuthSchema = Depends(AuthPermission(["demo:example:update"]))
) -> JSONResponse:
...
async def delete_obj_controller(
    ...
    auth: AuthSchema = Depends(AuthPermission(["demo:example:delete"]))
) -> JSONResponse:
...
async def batch_set_available_obj_controller(
    ...
    auth: AuthSchema = Depends(AuthPermission(["demo:example:patch"]))
) -> JSONResponse:
...
async def export_obj_list_controller(
    search: DemoQueryParam = Depends(),
    auth: AuthSchema = Depends(AuthPermission(["demo:example:export"]))
) -> StreamingResponse:
    # 获取全量数据
...
async def import_obj_list_controller(
    file: UploadFile,
    auth: AuthSchema = Depends(AuthPermission(["demo:example:import"]))
) -> JSONResponse:
...
@DemoRouter.post('/download/template', summary="获取示例导入模板", description="获取示例导入模板", dependencies=[Depends(AuthPermission(["demo:example:download"]))])
